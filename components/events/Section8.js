...
const Section8 = ({eventForm, setEventForm}) => {
    const handleForm = (e) => {
        setEventForm(previous => ({...previous, healthAreaOfFocusName: e.target.value}))
      }
  return (
    <div className="p-5 bg-extra-light-violet rounded">
      ...
      <div className="md:grid md:grid-cols-5 gap-x-16 gap-y-5">
        <label className="grid grid-cols-3 items-center p-2">
            <input type="radio" name="healthAreaOfFocusName" className='mr-2 w-6 h-6' onChange={handleForm}/>
        ...
        <label className="flex items-center p-2 col-start-2 col-end-6">
            ...
            <input type="text" name="healthAreaOfFocusName" className='w-full p-2 border-dark-violet rounded' placeholder="Please specify here"/>
        </label>
      </div>
    </div>
  );
