...
export default function EventsCardItems({programName,eventdate,eventName,urlEdit,urlParticipantSurvey,urlUpload,urlPostEventSurvey}) {
  return (
    <div className="events-card-item shadow-lg rounded-lg border-black p-5">
        ...
        <div className="event-card-item-program-date flex md:h-20  justify-between items-center border-black rounded-lg py-1">
          <div>
            <h3 className="pl-2">{eventName}</h3>
          </div>
          <div>
            <h3 className="pr-2">{eventdate && new Date(eventdate).toLocaleDateString('en-US',{year:'numeric',month:'numeric',day:'numeric'})}</h3>
          </div>
        </div>
